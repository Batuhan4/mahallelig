...
type Type = "walk" | "bike" | "stairs";
const LABEL: Record<Type, string> = {
  walk: tr.today.activityTypeWalk,
  bike: tr.today.activityTypeBike,
  stairs: tr.today.activityTypeStairs
};
const ICON: Record<Type, string> = { walk: "🚶", bike: "🚴", stairs: "🪜" };
        ...
        {(Object.keys(LABEL) as Type[]).map((t) => {
          ...
          return (
            <Pressable
              ...
              disabled={active}
              className={`flex-1 py-2.5 rounded-xl border ${isActive ? "bg-navy-900 border-navy-900" : "bg-transparent border-navy-900/15"} ${active && !isActive ? "opacity-40" : ""}`}
            >
              <Text
